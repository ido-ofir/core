...
module.exports = {
    ...
    setInputValue(fieldName, fieldValue){
      ...
      if(typeof fieldName === "object") {
        ...
        var afterChangeData = (() => {
          ...
          for(var key in o) {          
            d = setThis(key, o[key], d);            
          }  
          return d;
        })();
        this.setData(afterChangeData);
      } else {
        var afterChangeData = setThis(fieldName, fieldValue, { ...this.state.data });
        this.setData(afterChangeData);        
      }
    ...
    addInput(input) {
        ...
        if(!name) return console.warn(`[Form]: Input requires a name property`);
        if(this.inputs[name]) return console.warn(`[Form]: There are two inputs named ${name}`);
        this.inputs[name] = input;
        this.validate(true);
    },
    removeInput(input) {
        var name = input.props.name;
        if(!this.inputs[name]) return console.warn(`[Form]: Input ${name} tried to unregister but was not registered`);
