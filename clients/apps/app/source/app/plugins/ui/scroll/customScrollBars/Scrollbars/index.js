...
export default createClass({
    ...
    handleViewMouseMove(){
      this.showTracks();
      setTimeout(this.hideTracks, 600);
    },
    ...
    addListeners() {
        ...
        view.addEventListener('mousemove', this.handleViewMouseMove);
    ...
    removeListeners() {
        ...
        view.removeEventListener('mousemove', this.handleViewMouseMove);
        if (!getScrollbarWidth()) return;
    ...
    showTracks() {
        ...
        clearTimeout(this.hideTracksTimeout);
        if(trackHorizontal) css(trackHorizontal, { opacity: 1 });
        if(trackVertical) css(trackVertical, { opacity: 1 });
    },
    ...
    hideTracks() {
        ...
        this.hideTracksTimeout = setTimeout(() => {
            if(trackHorizontal) css(trackHorizontal, { opacity: 0 });
            if(trackVertical) css(trackVertical, { opacity: 0 });
        }, autoHideTimeout);
    },
